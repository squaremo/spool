...
RedisContext.prototype.createSignal = function(topic) {
  var signal;
...
HwmBuffer.prototype.timeWindow = function(since, until, callback) {
  since = since || '-inf';
  until = until || '+inf';
  var client = this._client;
  client.zrangebyscore(this._setkey, since, until, function(_, ids) {
    that._join(ids, callback);
  });
};

// in ascending order, but the offset is given from the most recent
// back (and zero-based); so, offset=10, limit=100 gives as many as
// 100 entries starting from the 11th most recent
HwmBuffer.prototype.rangeWindow = function(limit, offset, callback) {
  end = -1 - offset;
  start = limit && (end - limit) || 0;
  var client = this._client;
  client.zrange(this._setkey, start, end, function(_, ids) {
    that._join(ids, callback);
  });
};
